...
const haversine = (lat1, lon1, lat2, lon2) => {
  ...
  let a = Math.pow(Math.sin(dLat / 2), 2) + Math.cos(radians(lat1)) * 
  Math.cos(radians(lat2)) * Math.pow(Math.sin(dLon / 2), 2);
  ...
  switch(req.method){
    ...
    */
      ...
      for(let i = 0; i < valid.length - 1; i++){
        ...
        let dist = haversine(valid[i][0], valid[i][1], valid[i+1][0], valid[i+1][1])
